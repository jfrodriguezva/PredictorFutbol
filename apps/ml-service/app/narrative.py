"""Genera la narrativa "ultra detallada" en tono de experto/apostador.

Usa Claude (claude-opus-5) con la tool nativa de búsqueda web para incorporar
contexto reciente (lesiones, bajas, alineaciones) SOLO en partidos aún no jugados —
buscar noticias de un partido que ya terminó no aporta nada y puede confundir al
modelo. Si no hay ANTHROPIC_API_KEY, cae a una narrativa por plantilla construida
con los mismos datos cuantitativos, para que el producto funcione sin bloquear en
esa clave.
"""
...
def _fallback_narrative(
    ...
    lines += [
        ...
        "",
        "_Nota: configura ANTHROPIC_API_KEY en ml-service para obtener el análisis narrado "
        "completo de nuestro experto IA, incluyendo contexto de noticias recientes._",
    ]
    return "\n".join(lines)
...
def generate_narrative(
    ...
    is_upcoming = fixture.get("status") not in {"FT", "AET", "PEN", "CANC", "PST"}
    extra_kwargs: dict = {}
    if is_upcoming:
